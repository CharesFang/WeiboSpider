...
然后为初始化脚本`./init/init.sh`赋予权限后执行，创建用于存储数据的MongoDB Container.
...
Python脚本调用实质上也是通过CMD调用爬虫，可以方便爬虫的调试。在爬虫的**根目录下**（与`scrapy.cfg`文件处于同一目录下，可参见`run.py`）新建.py文件，并写入如下代码即可通过脚本调用爬虫。
